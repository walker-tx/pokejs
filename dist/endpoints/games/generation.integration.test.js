...
import { assert, assertType, beforeAll, describe, expect, expectTypeOf, it, vi, } from "vitest";
import { getGenerationById, getGenerationByName, listGenerations, } from "./generation.js";
import * as testData from "./generation.test-data.js";
describe("endpoints/games/generation", () => {
    beforeAll(() => {
        // type check the test data
        assertType(testData.getOneResult);
        assertType(testData.listResult);
    });
    ...
    describe("getGenerationByName", () => {
        it("should return the correct generation data when the API call is successful", async () => {
            const [data, error] = await getGenerationByName(testData.getOneResult.name);
        ...
        it("should return an http error when the API call fails", async () => {
            const [data, error] = await getGenerationByName("non-existent-generation");
...
//# sourceMappingURL=generation.integration.test.js.map